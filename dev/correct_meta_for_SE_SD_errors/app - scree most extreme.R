...
ui <- fluidPage(
  ...
  tags$h3("Scree-like plot of heterogeneity assuming N number of SE/SD errors by ranked absolute effect sizes"),
...
server <- function(input, output, session) {
  ...
  # Render the plot with H2 values across extremes
  ...
  # Render the plot with I2 values across extremes
  output$i2Plot <- renderPlot({
    metric_values <- compute_metrics_across_extremes()
    
    ggplot(metric_values, aes(x = n_extremes, y = I2)) +
      geom_line() +
      geom_point() +
      scale_x_continuous(breaks = scales::pretty_breaks(n = nrow(metric_values))) +  # Breaks at every integer
      labs(
        title = "I² Across Recalculated Extreme Studies",
        x = "Number of Most Extreme Studies Recalculated",
        y = "I²"
      ) +
      theme_linedraw()
