#' check_if_positive_definite
#'
#' Check if a correlation matrix is positive definite, ie whether the correlations are consistent with one another. This function takes a vector as input, and therefore is most useful for extracting and examining matrices extracted from articles.
#'
...
#'
#' @return logical indicating the matrix is positive-definite (TRUE) or non-positive-definite (FALSE)
#' 
#' @examples
...
check_if_positive_definite <- function(correlation_matrix_as_vector, is_lower_triangle, has_diagonal){
  ...
  # assess if positive definite, ie if all eigenvalues are > 0
  res <- all(eigen(mat)$values > 0)
